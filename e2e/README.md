...
| `gh-bvs-mirror` (moutai) | `forge-import PastaPastaPasta/backports-validation-script` (manual, PR G live check) | the same | A GitHub mirror: 3 branches + 2 PR heads, 1 issue, 2 PRs, 15 comments, 7 reviews, 9 labels. Re-running the import is a no-op. |
| `v1-m1-75299` (moutai) | `dg migrate …/m1-75299 --from-network testnet` (manual, PR G live check) | the same | The migrated copy of `m1-75299`: 25 refs, 121 packs, 29 issues, labels. Its clone matches the v1 clone ref for ref. |
