...
| `v1-m1-75299` (moutai) | `dg migrate …/m1-75299 --from-network testnet` (manual, PR G live check) | the same | The migrated copy of `m1-75299` (re-created 2026-09-26 on the re-registered contracts): 25 refs, 115 packs (4 without a public copy and 3 with no readable one are skipped; every ref's history passed the local check without them), 30 issues, 10 labels, 0.250 DASH. Its clone matches the v1 clone ref for ref, with the same HEAD tree, and passes fsck. |
