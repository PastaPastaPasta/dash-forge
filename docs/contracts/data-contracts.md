...
Branch/tag enumeration (no distinct-values query on Platform): **skip-scan with `limit 1` hops** — query `refNameHash > <last>` orderBy `refNameHash` **limit 1** to seek directly to the next distinct name (a `limit 100` page here would return *rows*, and one hyperactive ref with thousands of updates would make enumeration O(total pushes), not O(refs)); then batch tip lookups (`in` on ≤ 100 hashes with the §3 completeness fallback). Cost: one cheap seek per distinct ref — bounded by real branch/tag counts. Cursor-seek behavior verified in S0.8.

**`packManifest`** — `packHash` hash32; `kind` integer (**0 = git pack, 1 = objectLocator, 2 = flatIndex** — browse-plane artifacts share the entire pack storage/transport machinery); `sizeBytes`, `objectCount`, `chunkCount` integers; `storage` 0 platform / 1 external; `uris` array ≤ 8 of ≤ 300; `tips` ≤ 16 oid (for kind 2: the tip commit it indexes); `supersedes` ≤ 32 hash32; `offsetIndexParts` integer. Indices: unique `(packHash)`; `($createdAt desc)`; `(kind, $createdAt desc)` — readers grab newest locator/flatIndex in one query.
...
Browse-artifact formats (normative in `FORGE_RULES_V1`): **objectLocator** = fanout header (256 × u32 offsets) + oid-sorted `(oid, packRef, offset, length, deltaChainSpan)` rows, **~32 B/object** (oid 20 + packRef 2 + offset 5 + length 3 + span 2; ~3.2 MB total / ~12 KB per fanout slice for a 100k-object repo) — a lookup is header + one 1/256 slice via HTTP Range or chunk-seq range, and `deltaChainSpan` gives one contiguous read returning a blob with its delta bases (sound because stored packs are self-contained, above). **flatIndex** = tip commit oid + compressed path-sorted `(path, oid, mode, size)` rows — full recursive listing including gitlink (mode 160000) submodule entries. Both supersedable (old ones deleted for refund at next publish).
