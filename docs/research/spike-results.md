...
- ~~fix-thin premium: **0.9–4.4%** typical push (17% for a 100-commit batch).~~ **Superseded.** The self-contained-pack mandate is correct (raw thin packs do fail standalone `index-pack`), but self-containment is not sufficient: `--fix-thin` appends the materialized bases AFTER the deltas that use them, so the completed pack carries `REF_DELTA` and non-contiguous objects that the `objectLocator` must refuse. And the premium figure compares the completed pack against the *thin* pack, which is never stored — on this spike's 2.85 GiB corpus the thin baseline is 0.16–5.3 MB against 2–234 materialized bases, so the appended bytes read small; the same formula on a small repo's pushes is 150–440%. The push path now builds two locator-quality candidates and stores the smaller (`economics.md`).
