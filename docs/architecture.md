...
- Pack = unit of storage. **Stored packs are always self-contained and locator-quality**: `pack-objects --revs --delta-base-offset` over `want ^have` puts every delta base inside the pack, ahead of the deltas that use it, and the producer refuses anything else. (There is no wire step here, so no thin pack and no `--fix-thin` completion — that path stored *more* bytes and left `REF_DELTA` objects the browse index must refuse; see `economics.md`.) O(bytes/14 KiB) STs, not O(objects).
...
1. Helper resolves listing → repo contract; reads refs (proof-verified); builds **one self-contained, locator-quality pack** over the push delta.
...
4. Write `packManifest`, then `refUpdate` per ref (prevOid for force detection; non-FF refused without `+`), then publish the pack's **browse-index fragment** (§6.3) — best-effort: the push has already landed, and a repo whose index is behind still clones, fetches and pushes.
...
### Clone/fetch
Resolve listing → refs → collect non-superseded manifests covering want-set → fetch chunks (DAPI) or CID/URL (external) → SHA-256-verify reassembled pack → `git index-pack`. **Partial clone** via the `objectLocator` (bare-OID fetches + `.promisor` markers); **shallow clone is unsupported** — the helper has no reply channel for git's depth negotiation (S0.9), so `--depth` fails loudly. Local git odb is the cache (helper never re-fetches objects git has).
...
- Social artifacts are noise (2 KiB issue ≈ 2¢). Ref update ≈ 0.00008 DASH. Contract instantiation per repo ≈ **0.24 DASH** (S0.7-measured for a token contract; repo-v1 re-measured Stage 2).
